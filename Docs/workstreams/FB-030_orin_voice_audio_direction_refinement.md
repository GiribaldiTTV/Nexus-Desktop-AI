...
- FB-005 is now released and closed in `v1.6.6-prebeta`; release debt is clear; and FB-042 is now the active promoted implementation workstream on `feature/fb-042-desktop-entrypoint-runtime-refinement`, with Branch Readiness complete and WS-1 desktop shortcut launch-path runtime refinement admitted.
...
- Branch Readiness Gate: Complete on `feature/fb-042-desktop-entrypoint-runtime-refinement`; WS-1 desktop shortcut launch-path runtime refinement is admitted and active, while broader `main.py` reshaping plus broader workspace follow-through remain explicitly deferred until a later bounded seam
